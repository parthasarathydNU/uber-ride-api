...
def get_locations_service(request: Request) -> LocationService:
    return request.app.state.locations_service

@router.get("/locations", response_model=List[dict])
async def get_locations(locations_service: LocationService = Depends(get_locations_service)):
    """Get all locations"""
...
async def get_location(location_id: UUID, locations_service: LocationService = Depends(get_locations_service)):
...
async def create_location(location: CreateLocation, locations_service: LocationService = Depends(get_locations_service)):
...
async def update_location(location_id: UUID, updated_location: UpdateLocation, locations_service: LocationService = Depends(get_locations_service)):
...
async def delete_location(location_id: UUID, locations_service: LocationService = Depends(get_locations_service)):
    """Delete a location"""
...
async def get_nearest_location(latitude: float, longitude: float, locations_service: LocationService = Depends(get_locations_service)):
